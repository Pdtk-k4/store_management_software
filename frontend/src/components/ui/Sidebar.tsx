...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { key: "categoris", label: "Danh Mục", icon: "fas fa-tags", path: "#" },
  ...
  },
  {
    ...
    icon: "fas fa-users",
    children: [
      { key: "online-orders", label: "DS Nhân Viên", icon: "fas fa-user-friends", path: "#" },
      { key: "online-orders", label: "Thêm Nhân Viên", icon: "fas fa-user-plus", path: "#" },
      { key: "store-orders", label: "Chấm Công", icon: "fas fa-clock", path: "#" },
      { key: "edit-orders", label: "Lương Thưởng", icon: "fas fa-dollar-sign", path: "#" },
    ],
  },
];
...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  return (
    <>
      {/* Mobile Overlay (chỉ hiển thị khi mở sidebar) */}
      <div
        id="mobile-overlay"
      ...
      >
        ...
        {/* Navigation */}
        <nav className="p-4 space-y-2 overflow-y-auto h-full pb-20 custom-scrollbar">
          {menuItems.map((item) => item.children ? (
            <div key={item.key} className="space-y-1">
              {/* Nút toggle */}
              <button onClick={() => toggleMenu(item.key)} className="flex items-center justify-between w-full p-3 rounded-lg text-gray-800 text-base hover:bg-gray-100 transition-colors border-0 bg-white">
                <div className="flex items-center space-x-3">
